...
const initialProducts = [
  ...
  { id: 3, name: "Impact Wrench Half Inch Drive", sku: "VL-IW500", category: "Wrenches", stock: 67, price: 112, status: "In Stock", image: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=100" },
    ...
    <div className="min-h-screen bg-[#f5f7fa] flex">
      ...
      <div className="flex-1">
        <div className="bg-white border-b p-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4"><span>John Admin</span></div>
        </div>
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div><h1 className="text-2xl font-bold">Dashboard</h1><p className="text-sm text-gray-500">Welcome back - store performance overview</p></div>
            <div className="flex gap-2">
              <button onClick={()=>setShowAdd(true)} className="bg-[#2563eb] text-white px-4 py-2 rounded text-sm font-bold">+ Add Product</button>
              <button className="border px-4 py-2 rounded text-sm font-bold">Export</button>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white p-4 rounded-xl shadow border"><p className="text-xs text-gray-500">Total Sales</p><p className="text-2xl font-bold">$124,568</p><p className="text-xs text-green-600">+12.5% vs last month</p></div>
            <div className="bg-white p-4 rounded-xl shadow border"><p className="text-xs text-gray-500">Total Orders</p><p className="text-2xl font-bold">1,247</p><p className="text-xs text-green-600">+8.2%</p></div>
            <div className="bg-white p-4 rounded-xl shadow border"><p className="text-xs text-gray-500">Total Products</p><p className="text-2xl font-bold">342</p><p className="text-xs text-blue-600">+24 added</p></div>
            <div className="bg-white p-4 rounded-xl shadow border"><p className="text-xs text-gray-500">Low Stock Items</p><p className="text-2xl font-bold">14</p><p className="text-xs text-red-600">-3 from last week</p></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="col-span-2 bg-white p-4 rounded-xl shadow border">
              <h3 className="font-bold mb-4">Sales Overview</h3>
              <div className="h-32 flex items-end gap-2">{[10,15,12,18,22,25,22,38].map((h,i)=><div key={i} className="flex-1 bg-blue-500 rounded-t" style={{height: h*3}}></div>)}</div>
            ...
            <div className="bg-white p-4 rounded-xl shadow border"><h3 className="font-bold mb-4">Inventory Overview</h3><div className="flex justify-center"><div className="w-20 h-20 rounded-full border-8 border-blue-600"></div></div><div className="text-xs mt-4"><p>In Stock - 267</p><p>Low Stock - 48</p><p>Out of Stock - 27</p></div></div>
          </div>
          <div className="bg-white rounded-xl shadow border p-4">
            <h3 className="font-bold mb-4">Product Management</h3>
            <table className="w-full text-sm">
              ...
              <tbody>{products.slice(0,6).map((p,i)=><tr key={p.id} className="border-b"><td>{i+1}</td><td className="flex items-center gap-2 py-2"><img src={p.image} className="w-8 h-8 rounded" />{p.name}</td><td><span className="bg-yellow-100 text-xs px-2 py-1 rounded">{p.category}</span></td><td>{p.stock}</td><td>${p.price}</td><td>{p.status}</td></tr>)}</tbody>
            </table>
          </div>
        </div>
      </div>
      {showAdd && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            ...
            <div className="flex gap-2 mt-4"><button onClick={()=>setShowAdd(false)} className="flex-1 border py-2 rounded">Cancel</button><button onClick={addProduct} className="flex-1 bg-blue-600 text-white py-2 rounded">Save</button></div>
          </div>
        </div>
      )}
    </div>
